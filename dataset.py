...
class CleanNoisyPairDataset(Dataset):
    ...
    def __init__(self, root='./', subset='training', crop_length_sec=1):
        ...
        N_clean = len(os.listdir(os.path.join(root, 'training_set/urbansound8k/clean')))
        N_noisy = len(os.listdir(os.path.join(root, 'training_set/urbansound8k/noisy')))
        assert N_clean == N_noisy

        if subset == "training":
            self.files = [(os.path.join(root, 'training_set/urbansound8k/clean', 'fileid_{}.wav'.format(i)),
                           os.path.join(root, 'training_set/urbansound8k/noisy', 'fileid_{}.wav'.format(i))) for i in range(N_clean)]
        
        elif subset == "testing":
            sortkey = lambda name: '_'.join(name.split('_')[-2:])  # specific for dns due to test sample names
            _p = os.path.join(root, 'datasets/test_set/synthetic/no_reverb/urban_set')  # path for DNS
...
if __name__ == '__main__':
    import json
    with open('/data/users/tqiu5/CleanUNet/configs/DNS-large-high.json') as f:
        data = f.read()
